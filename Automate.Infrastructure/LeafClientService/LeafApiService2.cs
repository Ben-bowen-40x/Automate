...
public class LeafApiService2(IHttpClientFactory factory, ILeafApiSettings leafSettings, IDwhSettings settings) : IReportLeafMessageService
{
    #region Pathing
    // Parent folder
    ...
    public List<IMessage> GetMessages<T>(string messageLocation) where T : IMessageConvert
    {
        //// Retrieve Messages
        //// Create the client
        //HttpClient client = factory.CreateClient(leafSettings.LeafName!);
        //Task<HttpResponseMessage> responseTask = GetResponse(leafSettings, client);
        ...
        //List<T> messageCol = Response<T, >(response, out errMessage);
        ...
        //// Locals
        //static async Task<HttpResponseMessage> GetResponse(ILeafApiSettings leafSettings, HttpClient client)
        //{
        //    // Accept the response
        //    return await client.GetAsync(leafSettings.LeafBase);
        //}
        throw new NotImplementedException();
    }
    //static async Task<T?> ReadResult<T>(HttpResponseMessage response)
    //{
    //    return await response.Content.ReadFromJsonAsync<T>();
    //}
    //static List<T> Response<T>(HttpResponseMessage response, out string errMessage) where T : IMessageConvert
    //{
    //    try
    //    {
    //        if (response.IsSuccessStatusCode)
    //        {
    //            Task<T?> value = ReadResult<T>(response);
