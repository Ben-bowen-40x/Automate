...
public class TypedDiscrepancyManager(ITypedDiscrepancyService discrepancyService, IReportService reportService) : ITypedDiscrepancyManager
{
    ...
    public Result<FileInfo> Manage<T>(FileInfo billedCalls, FileInfo comparisonCalls, string reportLoc) where T : IConvert
    {
        // Retrieve billed leads
        List<IDiscrepancyCall> billed = _discrepancy.GetCalls<T>(billedCalls);

        // Retrieve comparisons
        List<IDiscrepancyCall> comparison = _discrepancy.GetCalls<T>(comparisonCalls);
        ...
        Result<FileInfo> result = _reporting.GenerateDiscrepancyReport(analyzed, reportLoc);
